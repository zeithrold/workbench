...
class ManagePermissionPolicyControllerTest(@Autowired private val mockMvc: MockMvc) {
  ...
  class TestBeans {
    ...
    fun permissionPolicyRepositorySetup(policies: PermissionPolicyRepository): Boolean {
      ...
      val tenantReadRule =
        one.ztd.workbench.identity.permission.PermissionPolicyRuleRecord(
          id = UUID.randomUUID(),
          apiId = PublicId.new("prr"),
          policyId = customPolicyId,
          action = one.ztd.workbench.identity.permission.model.AuthorizationAction("tenant.read"),
          resourcePattern = "tenant:*",
      ...
      val tenantUpdateRule =
        tenantReadRule.copy(
          id = UUID.randomUUID(),
          apiId = PublicId.new("prr"),
          action = one.ztd.workbench.identity.permission.model.AuthorizationAction("tenant.update"),
          effect = one.ztd.workbench.identity.permission.model.PermissionEffect.DENY,
        )
      var rules: List<one.ztd.workbench.identity.permission.PermissionPolicyRuleRecord> =
      ...
      coEvery { policies.addRule(any()) } answers
        {
          val command =
            firstArg<one.ztd.workbench.identity.permission.CreatePermissionPolicyRuleCommand>()
          val rule =
