    ...
    template.fields.forEach { (field, spec) ->
      ...
      val effective =
        reconcileField(
          ...
          userValue = userValue,
          canWrite =
            fieldPermissions.canWriteField(
              permissionContext.copy(operation = FieldPermissionOperation.UPDATE),
              field,
            ),
        )
      ...
      if (spec.participation == FieldParticipation.REQUIRED && !effective.isNonNullValue()) {
        if (spec.writeGrant == FieldWriteGrant.IMMUTABLE) {
          throw InvalidRequestException(
            ...
            "Required transition field is immutable and has no value: ${field.toWirePath()}",
          )
        }
        throw InvalidRequestException(
          WorkbenchErrorCode.WORK_ITEM_PROPERTY_REQUIRED,
          "Required transition property is missing: ${field.toWirePath()}",
        )
      }
    ...
    return when (spec.writeGrant) {
      ...
      FieldWriteGrant.INHERIT ->
        if (canWrite) {
          ...
          handleUnauthorized(spec, currentValue, templateValue, userSubmitted)
        }
    }
  }
