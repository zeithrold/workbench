  ...
  suspend fun retryCloseOperation(
    ...
    val operation =
      closeOperations.findByApiId(tenantId, projectId, sprintApiId, operationApiId)
        ?: throw ResourceNotFoundException(WorkbenchErrorCode.SPRINT_CLOSE_OPERATION_NOT_FOUND)
    if (
      operation.status != one.ztd.workbench.agile.sprint.model.SprintCloseOperationStatus.FAILED
    ) {
...
private fun validateCloseTarget(
  ...
  val expectsTarget =
    command.disposition == one.ztd.workbench.agile.sprint.model.SprintCloseDisposition.NEXT_SPRINT
...
data class SprintCloseOperationView(
  ...
  companion object {
    fun from(record: one.ztd.workbench.agile.sprint.model.SprintCloseOperationRecord) =
      SprintCloseOperationView(
        id = record.apiId.value,
