...
import one.ztd.workbench.agile.testfixtures.TransitionTestHarness
...
class WorkItemTransitionServiceTest :
  StringSpec({
    ...
    fun stubTransitionContext(
      ...
      config: one.ztd.workbench.agile.workitem.model.IssueTypeConfigDetails,
    ) {
    ...
    "transition executes and returns mutation result" {
      runTest {
        ...
        val transition = AgileWorkItemFixtures.sampleTransition(config)
        val mutationResult =
          one.ztd.workbench.agile.workitem.model.WorkItemMutationResult(
            issue,
