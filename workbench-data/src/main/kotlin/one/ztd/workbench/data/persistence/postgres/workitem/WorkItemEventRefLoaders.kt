...
import one.ztd.workbench.data.persistence.postgres.findColumn
...
internal fun loadUserEntityRef(userId: UUID): WorkItemActivityEntityRef? {
  val row =
    one.ztd.workbench.data.persistence.postgres.identity.UsersTable.selectAll()
      .where {
        one.ztd.workbench.data.persistence.postgres.identity.UsersTable.id eq userId.toKotlinUuid()
      }
      .singleOrNull() ?: return null
  return WorkItemActivityEntityRef(
    id = row[one.ztd.workbench.data.persistence.postgres.identity.UsersTable.apiId],
    display = row[one.ztd.workbench.data.persistence.postgres.identity.UsersTable.displayName],
  )
}
