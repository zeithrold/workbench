...
<div class='space-y-4'>
  ...
  {#if loading}<LoadingState label={m.management_loading_permission_groups()} />{:else}
    <div class='grid gap-5 lg:grid-cols-[22rem_1fr]'>
      <div class='space-y-3'>
        {#if canEdit}<Card><CardContent class='grid gap-2 p-4'><Label for='group-code'>{m.management_new_custom_group()}</Label><Input id='group-code' placeholder={m.management_code()} bind:value={code} /><Input placeholder={m.management_name()} aria-label={m.management_group_name()} bind:value={name} /><Button disabled={!code || !name} onclick={createGroup}>{m.management_create_group()}</Button></CardContent></Card>{/if}
      ...
      {#if selected}<Card><CardContent class='space-y-5 p-5'><div class='flex items-start justify-between'><div><h3 class='text-lg font-semibold'>{selected.name}</h3><p class='text-sm text-muted-foreground'>{selected.description ?? m.management_no_description()}</p></div>{#if canEdit && !selected.builtin}<Button variant='ghost' size='sm' onclick={deleteSelectedGroup}>{m.management_delete_group()}</Button>{/if}</div>
        {#if canEdit && !selected.builtin}<div class='flex gap-2'><select class='h-9 min-w-0 flex-1 rounded-md border bg-background px-3 text-sm' bind:value={userId}><option value=''>{m.management_select_tenant_member()}</option>{#each members.filter(member => !groupMembers.some(item => item.user.id === member.user.id)) as member (member.user.id)}<option value={member.user.id}>{member.user.displayName}</option>{/each}</select><Button disabled={!userId} onclick={addMember}>{m.management_add_member()}</Button></div>{/if}
        <div class='divide-y rounded-md border'>{#each groupMembers as member (member.id)}<div class='flex items-center justify-between p-3'><div><p class='text-sm font-medium'>{member.user.displayName}</p><p class='text-xs text-muted-foreground'>{member.user.primaryEmail}</p></div>{#if canEdit && !selected.builtin}<Button variant='ghost' size='sm' onclick={() => perform(() => managementGateway.removeGroupMember(selected.id, member.user.id))}>{m.remove()}</Button>{/if}</div>{:else}<p class='p-5 text-center text-sm text-muted-foreground'>{m.management_no_group_members()}</p>{/each}</div>
      </CardContent></Card>{/if}
    </div>
  {/if}
</div>
