<script lang='ts'>
  ...
  import { AccessDeniedState, Alert, Badge, Button, Card, CardContent, Input, Label, LoadingState, SearchableSelect } from '$lib/shared/ui'
...
<div class='space-y-4'>
  ...
  {:else}
    ...
    {#if loading}<LoadingState label={m.management_loading_permission_bindings()} />{:else}
      <Card><CardContent class='grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-4'><div class='space-y-1'><Label>{m.management_principal_type()}</Label><SearchableSelect value={principalType} options={principalTypeOptions} clearable={false} onValueChange={selectPrincipalType} /></div>
      ...
      <div class='space-y-2'>{#each bindings as binding (binding.id)}<div class='flex flex-wrap items-center justify-between gap-3 rounded-lg border bg-background p-4'><div><div class='flex items-center gap-2'><p class='font-medium'>{binding.policy.name}</p>{#if binding.validTo && new Date(binding.validTo) <= new Date()}<Badge variant='secondary'>{m.management_expired()}</Badge>{/if}</div><p class='text-sm text-muted-foreground'>{binding.validTo ? m.management_binding_summary_until({ principal: binding.user?.displayName ?? binding.group?.name ?? m.management_all_tenant_members(), date: new Date(binding.validTo).toLocaleString() }) : m.management_binding_summary({ principal: binding.user?.displayName ?? binding.group?.name ?? m.management_all_tenant_members() })}</p></div><Button variant='ghost' size='sm' onclick={() => perform(() => managementGateway.deleteBinding(binding.id))}>{m.management_expire()}</Button></div>{:else}<p class='rounded-lg border border-dashed p-8 text-center text-sm text-muted-foreground'>{m.management_no_permission_bindings()}</p>{/each}</div>
    {/if}
  {/if}
</div>
