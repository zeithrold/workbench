<script lang='ts'>
  ...
  async function load() {
    loading = true; try { tenants = await managementGateway.listTenants(); loaded = true }
...
<div class='space-y-8'>
  ...
  {#if !loaded && error && isApiProblemStatus(error, 403)}
    <AccessDeniedState description={error.message} />
  {:else}
