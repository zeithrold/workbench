  ...
  >
    <div class='flex h-11 items-center gap-2 border-b border-border/60 px-3'>
      {#each [0, 1, 2, 3, 4, 5] as item (item)}
        <Skeleton class='size-7 shrink-0' label='Loading editor control' />
      {/each}
    </div>
    <div class='grid min-h-56 content-start gap-3 p-7'>
      <Skeleton class='h-4 w-4/5' label='Loading editor content' />
      <Skeleton class='h-4 w-full' label='Loading editor content' />
      <Skeleton class='h-4 w-3/5' label='Loading editor content' />
    </div>
  </div>
{/snippet}

<AsyncComponent loader={loadEditor} componentProps={props} loading={editorSkeleton} loadingLabel='Loading rich text editor' />
