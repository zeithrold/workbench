/**
 * Generated by orval v8.20.0 🍺
 * Do not edit manually.
 * Workbench API
 * Multi-tenant work management API. Use X-Workbench-API-Version for date-based API versioning. Successful responses may include X-Workbench-Warning for non-blocking business risks.
 * OpenAPI spec version: 2026-07-13
 */
import type { JsonNode } from './jsonNode';
import type { TypeConfigPropertyRequest } from './typeConfigPropertyRequest';
import type { TypeConfigStatusRequest } from './typeConfigStatusRequest';

export interface CreateIssueTypeConfigRequest {
  /** @minLength 1 */
  scope: string;
  projectId?: string;
  /** @minLength 1 */
  issueTypeId: string;
  /** @minLength 1 */
  workflowId: string;
  nameOverride?: string;
  iconOverride?: string;
  colorOverride?: string;
  rank?: number;
  createFields?: JsonNode;
  statuses?: TypeConfigStatusRequest[];
  properties?: TypeConfigPropertyRequest[];
}
