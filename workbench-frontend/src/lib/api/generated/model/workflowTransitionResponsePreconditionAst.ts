/**
 * Generated by orval v8.20.0 🍺
 * Do not edit manually.
 * Workbench API
 * Multi-tenant work management API. Use X-Workbench-API-Version for date-based API versioning. Successful responses may include X-Workbench-Warning for non-blocking business risks.
 * OpenAPI spec version: 2026-07-13
 */
import type { JsonElement } from './jsonElement';
import type { WorkflowTransitionResponsePreconditionAstEntriesItem } from './workflowTransitionResponsePreconditionAstEntriesItem';

export type WorkflowTransitionResponsePreconditionAst = {
  values?: JsonElement[];
  empty?: boolean;
  size?: number;
  entries?: WorkflowTransitionResponsePreconditionAstEntriesItem[];
  keys?: string[];
  [key: string]: unknown;
};
