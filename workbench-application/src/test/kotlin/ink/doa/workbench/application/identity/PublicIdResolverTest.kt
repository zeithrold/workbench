...
class PublicIdResolverTest :
  StringSpec({
    ...
    "resolveProject returns project for tenant" {
      ...
      val project =
        ink.doa.workbench.agile.project.model.ProjectRecord(
          id = projectId,
    ...
    "resolveLoginMethod returns login method record" {
      val method =
        ink.doa.workbench.identity.model.LoginMethodDefinitionRecord(
          ...
          code = "password",
          kind = ink.doa.workbench.identity.model.LoginMethodKind.PASSWORD,
          name = "Password",
          isBuiltin = true,
    ...
    "resolveBearerToken returns token record" {
      val token =
        ink.doa.workbench.identity.model.BearerTokenRecord(
          id = UUID.randomUUID(),
    ...
    "resolveAdminUser and resolveAccessGrant return records" {
      val adminUser =
        ink.doa.workbench.identity.permission.AdminUserRecord(
          id = UUID.randomUUID(),
          apiId = PublicId("adm_01JABCDEFGHJKMNPQRSTVWXYZ5"),
          userId = UUID.randomUUID(),
          scope = ink.doa.workbench.identity.permission.AdminScope.TENANT,
          tenantId = UUID.randomUUID(),
          status = ink.doa.workbench.identity.permission.AdminUserStatus.ACTIVE,
      ...
      val grant =
        ink.doa.workbench.identity.permission.AccessGrantRecord(
          id = UUID.randomUUID(),
