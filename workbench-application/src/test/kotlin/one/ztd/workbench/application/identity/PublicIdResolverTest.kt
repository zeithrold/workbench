...
class PublicIdResolverTest :
  StringSpec({
    ...
    "resolveProject returns project for tenant" {
      ...
      val project =
        one.ztd.workbench.agile.project.model.ProjectRecord(
          id = projectId,
    ...
    "resolveLoginMethod returns login method record" {
      val method =
        one.ztd.workbench.identity.model.LoginMethodDefinitionRecord(
          ...
          code = "password",
          kind = one.ztd.workbench.identity.model.LoginMethodKind.PASSWORD,
          name = "Password",
          isBuiltin = true,
    ...
    "resolveBearerToken returns token record" {
      val token =
        one.ztd.workbench.identity.model.BearerTokenRecord(
          id = UUID.randomUUID(),
    ...
    "resolveAdminUser and resolveAccessGrant return records" {
      ...
      val grant =
        one.ztd.workbench.identity.permission.AccessGrantRecord(
          id = UUID.randomUUID(),
          ...
          action = one.ztd.workbench.identity.permission.model.AuthorizationAction("project.read"),
